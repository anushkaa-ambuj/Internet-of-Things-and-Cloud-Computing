...
node = sx126x.sx126x(serial_num="/dev/ttyS0", freq=868, addr=7, power=22, rssi=True, air_speed=2400, relay=False)

def send_deal(recieve_addr, freq, send_data):
    get_rec = ""
    print("")
    print("Sending data automatically...")
    
    get_t = [recieve_addr, freq, str(send_data)]
    print(get_t)
...
try:
    ...
    while True:
        send_deal(recieve_addr, freq, send_data)  # Call the function to send data continuously
